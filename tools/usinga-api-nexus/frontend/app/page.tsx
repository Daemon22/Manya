      ...
      <header className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-5 py-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="inline-flex h-10 items-center gap-2 rounded-md bg-signal px-3 text-sm font-semibold text-white shadow-sm">
              <DatabaseZap size={17} />
        ...
        <aside className="h-fit border-r border-line bg-transparent pr-4">
          <nav className="grid gap-1 text-sm">
            {[
              ...
              <button
                key={label as string}
                className="flex h-10 items-center gap-2 rounded-md px-3 text-left font-medium text-slate-700 hover:bg-white hover:shadow-sm"
              >
                <Icon size={17} />
                {label as string}
              </button>
        ...
        <section className="grid gap-5">
          ...
          <div className="grid gap-5 xl:grid-cols-[1.4fr_0.9fr]">
            <section className="rounded-lg border border-line bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-600">Health, cost, latency, and feature coverage in one scan.</p>
                </div>
                <select
            ...
            <section className="rounded-lg border border-line bg-white p-5 shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-600">The backend scores provider health, key availability, cost, latency, and budget.</p>
            ...
            <section className="rounded-lg border border-line bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 grid gap-3">
                {auditEvents.map((event) => (
                  <div key={event} className="flex items-start gap-3 rounded-md border border-line bg-panel p-3 text-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-4">
              {["Marketplace", "Team collaboration", "Local model orchestration", "OMNIMIND integration"].map((item) => (
                <div key={item} className="rounded-md border border-line bg-panel p-4">
                  ...
                  <p className="mt-1 text-sm text-slate-600">Planned after the wallet, monitoring, analytics, and routing foundation.</p>
                </div>
              ))}
            </div>
          </section>
...
function FormRow({ label, value }: { label: string; value: string }) {
  return (
    <label className="grid gap-1 text-sm">
      ...
      <input className="h-10 rounded-md border border-line bg-white px-3 text-ink" readOnly value={value} />
    </label>
  );
}
