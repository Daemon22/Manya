 ...
 *     the value index.
 *   - Bus events are ephemeral (never persisted).
 */
...
import {
  ...
  mergeIdentities,
  listIdentities,
  identityCount as _identityCount,
  _resetFederation,
  _hydrateIdentities,
  // Event bus
  createBus,
...
import { loadState, saveState, resetState, statePath as _statePath } from './state.js';
...
export async function runCommand(parsed, context = {}) {
  const _proc = context.process || process;
  ...
  async function persist(path) {
    ...
    const identities = listIdentities();
    saveState({ tools, identities }, path);
  }
}
